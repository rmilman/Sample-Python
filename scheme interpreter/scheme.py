...
def make_call_frame(procedure, args, env):
    ...
    if isinstance(procedure, MuProcedure):
        ...
        if len(formals) != len(args):
            raise SchemeError("Number of values and formals must match")
        i = 0
        for f in formals:
            frame.define(f, args[i])
            i += 1
        return frame
...
def do_cond_form(expressions, env):
    ...
    for i, clause in enumerate(expressions):
        ...
        if scheme_true(test) and clause.second != nil:
            return eval_all(clause.second, env)
        elif scheme_true(test):
            return test   
    return okay
...
def make_let_frame(bindings, env):
    ...
    for binding in bindings:
        ...
        if scheme_symbolp(binding.first):
            ...
            vals.append(scheme_eval(binding.second.first,env))
        else:
            raise SchemeError
...
def check_formals(formals):
    ...
    """
    if not scheme_listp(formals): #check if list
        raise SchemeError("Formals must be a well-formed Scheme list")
    for i in range(len(formals)): #check
        if not scheme_symbolp(formals[i]):
            raise SchemeError("Formals must be symbols")
    if len(set(formals)) < len(formals):
       raise SchemeError("Repeated formals")
...
class MuProcedure(UserDefinedProcedure):
    ...
    # def muke_child_frame(self, env, formals, vals):
    #     """Return a new local frame whose parent is SELF, in which the symbols
    #     in a Scheme list of formal parameters FORMALS are bound to the Scheme
    #     values in the Scheme list VALS. Raise an error if too many or too few
    #     vals are given.
    ...
    #     <{a: 1, b: 2, c: 3} -> <Global Frame>>
    #     """
    #     frame = Frame(env) # Create a new frame with current env as the parent
    #     if len(formals) != len(vals):
    #         raise SchemeError("Number of values and formals must match")
    #     i = 0
    #     for f in formals:
